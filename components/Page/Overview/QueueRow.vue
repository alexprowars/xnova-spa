<template>
	<div class="row flight">
		<div class="th col-4 col-sm-2">
			...
			<span class="positive d-sm-none">{{ dayjs(item['time']).format('DD MMM HH:mm:ss') }}</span>
		</div>
		<div class="th col-sm-10 col-8 text-start">
			...
			<span class="positive float-sm-end d-none d-sm-inline">{{ dayjs(item['time']).format('DD MMM HH:mm:ss') }}</span>
		</div>
	</div>
</template>
